...
export default verifyRequestAuthThen(async ({ req, token }) => {
  try {
    try {
      var { refresh_token } = await req.json() as Record<string, unknown>;
    } catch (_) {
      error(`${authRefreshToken} json error:`);
      return badRequest();
    }
    ...
    if (
      ...
      error(`${authRefreshToken} validation error:`, {
    ...
    if (response.error) {
      error(`${authRefreshToken} response error:`, { error: response.error });
    }
    ...
    error(`${authRefreshToken} error:`, { error: err });
  }
